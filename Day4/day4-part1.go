...
func main() {
	...
	maxId := findMaxId(sleepingTimes)
	fmt.Printf("ID with max sleep time %d\n", maxId)

	maxIdsSleepingTimes := sleepingPeriods[maxId]

	bestMinutes := map[int]int{}
	for _, time := range maxIdsSleepingTimes {
	...
	fmt.Printf("Best minute is %d\n", findMaxId(bestMinutes))
}
